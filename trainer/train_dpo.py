...
def logits_to_log_probs(logits, labels):
    ...
    # log_probs 计算每个token的 的 log-softmax 概率
    # 也就是先softmax计算每个token的vocab_size维度的概率分布，再取对数得到log_probs，为了计算损失函数用的
    log_probs = F.log_softmax(logits, dim=2)
    # 通过gather函数根据labels索引出每个token对应的log概率，得到log_probs_per_token shape: (batch_size, seq_len
...
def dpo_loss(ref_log_probs, policy_log_probs, mask, beta):
    ...
    # ─── Step 1: 对 response token 的 log-prob 做 mask + 归一化 ───
    seq_lengths = mask.sum(dim=1, keepdim=True).clamp_min(1e-8)  # 防止零长度mask导致除零NaN
    # 对每个样本的log概率求和，并除以有效token数量，得到每个样本的平均log概率，shape: (batch_size,)
    ...
    #  pi_logratios  = log π_θ (y_w|x) - log π_θ (y_l|x)
...
def train_epoch(epoch, loader, iters, ref_model, lm_config, start_step=0, wandb=None, beta=0.1):
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        # 为啥可以拼接？上下文内容不就变了嘛
        # 这里是在batch的维度上拼接的，不实在序列长度维度上拼接，所以上下文内容不会变，每个样本还是独立的
        # 只是把两批样本拼接在一块，一起送进模型，纯粹为了提高效率
        ...
        with autocast_ctx:
            # 计算参考模型的log概率，ref_model不更新，所以用torch.no_grad()包裹
            with torch.no_grad():
